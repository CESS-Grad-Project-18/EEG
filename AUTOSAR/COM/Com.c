...
static uint8 Com_DataBuffer[COM_MAX_BUFF_LEN];
...
void Com_Init(const Com_ConfigType *config){
	...
	for (i = 0; !ComConfig->ComIPdu[i].Com_EOL; i++) {
		...
		IPdu->ComIPduDataPtr = (void *)&Com_DataBuffer[bufferIndex];
...
/* Helper functions */
/* Copies the N LSB from the Signal into PDU.
 * Bit segment is placed into PDU so that MSB is placed at start bit offset from the MSB of the PDU */
...
void Com_WriteToPDU(Com_SignalIdType signalId, const void *signalData, boolean *dataChanged){
	...
	uint8 signalLength = Com_Ceil(Signal->ComBitSize, 8);
