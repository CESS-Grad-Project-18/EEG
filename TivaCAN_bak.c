...
void CAN0_Init(void){
  SYSCTL_R_RCGC2_R |= 0x00000002;   /* 1) Provide clock to Port B (clock gating control) */
  SYSCTL_RCGC0_R |= 0x01000000;     /* 2) Provide clock to Enable Clock for CAN0 Module */
  ...
  CAN0_BIT_R = 0x34C9; /* 500kbps baud rate */
  CAN0_CTL_R &= ~0x41; /* No intr, CAN is enabled (Init = 0) */
}
...
void CAN0_WriteSet(int msgObjNo){
    ...
    CAN0_IF1CMSK_R = 0xB3; /* WRNRD, CONTROL, DATAA, DATAB, ARB bits set */
    CAN0_IF1MSK1_R = 0x0;
    CAN0_IF1MSK2_R = 0x0;
    CAN0_IF1ARB1_R = 0x0;
    CAN0_IF1ARB2_R = 0xA424; /* DIR bit set. MSG Valid bit set in ARB2 reg, MSG ID set to 0x424 */
    CAN0_IF1MCTL_R = 0x188; /* TXRQST bit set, set # of data bytes */
...
void CAN0_Write(int msgObjNo, char data[8], int debug){
    ...
    CAN0_IF1CRQ_R = msgObjNo; /* Message object number */
    while(CAN0_IF1CRQ_R & 0x8000); /* Wait for BUSY bit to clear */
...
void CAN0_Read(int msgObjNo, char* data, int debug){
    ...
    while(CAN0_IF2CRQ_R & 0x8000); /* Wait while CAN0 Module is BUSY */
...
int main(void){
    tCANMsgObject msgObjectTx_5;
    uint8_t msgObjectTx_5_Data = 5;
    msgObjectTx_5.ui32MsgID = 265;
    msgObjectTx_5.ui32MsgIDMask = 0;
    msgObjectTx_5.ui32MsgLen = sizeof(msgObjectTx_5_Data);
    msgObjectTx_5.pui8MsgData = (uint8_t *)&msgObjectTx_5_Data;
    PLL_Init_50MHz();
    CAN0_Init();
    UART0_Init();
    UART5_Init();
    ...
    CANEnable(CAN0_BASE);
    CANMessageSet(CAN0_BASE, 1, &msgObjectTx_5, MSG_OBJ_TYPE_TX);
    CAN0_WriteSet(1);
    GPIO_PORTF_DATA_R = 0x00;
    UART5_WriteString("r - For Red LED");
